            ...
            <div class="w-full bg-white rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form method="POST" action="{{ route('login') }}" class="space-y-4 md:space-y-6">
                        @csrf
                        <div>
                            <input type="hidden" name="tipo_login" id="tipo_login" value="{{'site'}}" />
                            <label for="email" class="block mb-2 text-sm font-medium text-gray-900">Seu Email</label>
                            <input type="email" name="email" id="email"
                                class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5"
                                placeholder="larusso@miyagido.com" required autocomplete="username" />
                        </div>
                        <div>
                        ...
                        <div class="flex items-center justify-between">
                            <a href="esqueci_senha" class="text-sm font-medium text-primary-600 hover:underline">Esqueceu sua
                                senha?</a>
                        </div>
                        <button type="submit"
